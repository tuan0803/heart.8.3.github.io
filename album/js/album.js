(async function () {
  ...
  try {
    ...
    if (archiveInfo.description)
      document.title += `- ${archiveInfo.description}`;

    document.querySelector("h1").innerHTML =
      archiveInfo.title || "<em>Untitled</em>";
    ...
    updatePrompt(
      '<p>Something went wrong.</p><a href="https://github.com/beakerbrowser/dat-photos-app">Report an issue</a>'
    );
  }
  ...
  function onShare() {
    ...
    updatePrompt(
      `<div id="close-prompt">Close</div><p>Share your photo album's secret URL:</p><p><code>${archive.url}</code></p><p><em>URL copied to clipboard</em></p>`
    );
  ...
  async function renderAlbum() {
    try {
      ...
      for (let i = 0; i < paths.length; i++) {
        ...
        const orientation = window.localStorage.getItem(
          `${archive.url}${path}`
        );
        appendImage(path, orientation);
      }
  ...
  function renderUAPrompt() {
    updatePrompt(
      '<p>Sorry >.< This app only works in the Beaker Browser.</p><a class="btn primary" href="https://beakerbrowser.com/docs/install/">Install Beaker</a>'
    );
  }
  ...
  function updatePrompt(html) {
    ...
    if (html.length) {
      document.querySelector(
        "#prompt"
      ).innerHTML = `<div class="content">${html}</div>`;
    } else {
