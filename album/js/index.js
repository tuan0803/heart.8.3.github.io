(async function () {
  ...
  async function onDeleteAlbum(e) {
    ...
    document
      ...
      .removeChild(document.querySelector(`a.album[href="${url}"]`));
  ...
  function renderApp() {
    ...
    document
      .querySelectorAll(".create-album")
      .forEach((el) => el.addEventListener("click", onCreateAlbum));
  ...
  async function appendAlbum(album) {
    ...
    // add the title
    albumHTML += `<div class="title">${
      info.title || "<em>Untitled</em>"
    }</div>`;
  ...
  function toggleAlbumDropdown(e) {
    ...
    document
      .querySelector(`.dropdown[data-album="${e.target.dataset.album}"]`)
